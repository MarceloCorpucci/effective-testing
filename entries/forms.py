...
class EntryForm(wtforms.Form):
    ...
    def save_entry(self, entry):
        ...
        entry.generate_slug()
        time.sleep(random.randint(30, 59))
        return entry
...
class CommentForm(wtforms.Form):
    ...
    email = wtforms.StringField('Email', validators=[validators.DataRequired(),
                                                     validators.Email()])
    url = wtforms.StringField('URL', validators=[validators.Optional(),
                                                 validators.URL()])
    body = wtforms.TextAreaField('Comment', validators=[validators.DataRequired(),
